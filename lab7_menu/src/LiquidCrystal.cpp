...
void delayMicroseconds(unsigned int us){
	...
	uint64_t val = (uint64_t) us * Chip_Clock_GetSystemClockRate()/MICRO;

	// disable RIT – compare value may only be changed when RIT is disabled
	Chip_RIT_Disable(LPC_RITIMER);

	// set compare value to RIT
	Chip_RIT_SetCompareValue(LPC_RITIMER, val);
...
void LiquidCrystal::print(std::string const &s)
{
	...
	for(std::string::const_iterator it = s.begin(); it != s.end(); ++it){
		if(*it != '\n'){ // if it reads a new line, print on the next line
			write(*it);
		} else if( i < 16){
			i=15; // 15 since it will increment once more
		}
		i++;
		if(i==16){
			setCursor(0,1);
	...
	setCursor(0,0); // Reset the cursor, otherwise it'll will continue to write on the second line
	//print(s.c_str());
}
...
void LiquidCrystal::print(const char *s)
{
	...
	while(*s != '\0'){
		...
		i++;
		if(i==16){
			setCursor(0,1); // Reset the cursor, otherwise it'll will continue to write on the second line
		}
	}
	/*
	for(int i=0; s[i]; i++){
